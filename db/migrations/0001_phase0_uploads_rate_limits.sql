...
-- counters (usage_counters). New tables only. Idempotent (safe to re-run):
-- production was evolved with `drizzle-kit push`, so this file may be applied
-- by hand. Apply before 0002_clerk_identity.sql.
--
...
DO $$ BEGIN
  CREATE TYPE "public"."blob_upload_status" AS ENUM('pending', 'uploaded', 'linked', 'rejected');
EXCEPTION
...
--> statement-breakpoint
DO $$ BEGIN
  ALTER TABLE "blob_uploads" ADD CONSTRAINT "blob_uploads_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
--> statement-breakpoint
DO $$ BEGIN
  ALTER TABLE "blob_uploads" ADD CONSTRAINT "blob_uploads_resource_id_resources_id_fk" FOREIGN KEY ("resource_id") REFERENCES "public"."resources"("id") ON DELETE set null ON UPDATE no action;
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
--> statement-breakpoint
...
END $$;
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "blob_uploads_user_idx" ON "blob_uploads" USING btree ("user_id","created_at");
