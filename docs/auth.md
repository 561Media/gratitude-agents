...
7. **Database:** take a Neon branch or backup, then apply `db/migrations/0001_clerk_identity.sql` to production (idempotent; adds nullable `clerk_user_id`, makes `password_hash` nullable). Do not use `drizzle-kit migrate`: production was built with `push` and has no migration history table.
...
- **Database:** migration 0001 is backward compatible. The old code ignores `clerk_user_id`, and existing password hashes are untouched. Only people invited after cutover (no password) cannot use the old login.
